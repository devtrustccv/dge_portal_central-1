...
export async function getOfertaFormativaByMeiliSearch({
  ...
  filterObject,
    ...
    try {
        ...
        if (filterObject?.entidade) {
            const entidades = filterObject.entidade.split(",");
            filter.push(`(${entidades.map(e => `denominacao_entidade='${e}'`).join(" OR ")})`);
        }

        if (filterObject?.nivel) {
            const niveis = filterObject.nivel.split(",");
            filter.push(`(${niveis.map(e => `nivel='${e}'`).join(" OR ")})`);
        }

        if (filterObject?.familia) {
            const familiaprofissional = filterObject.familia.split(',');
            filter.push(`(${familiaprofissional.map(e => `familia='${e}'`).join(" OR ")})`);
        }

        if (filterObject?.concelho) {
            const ilha_concelho = filterObject.concelho.split(",");
            filter.push(`(${ilha_concelho.map(e => `concelho='${e}'`).join(" OR ")})`);
        }

        if (filterObject?.modalidade) {
            const modalidades = filterObject.modalidade.split(",");
            filter.push(`(${modalidades.map(e => `modalidade='${e}'`).join(" OR ")})`);
        }

        if (filterObject?.periodo_formacao) {
            const periodo_formacao = filterObject.periodo_formacao.split(",");
            filter.push(`(${periodo_formacao.map(e => `periodo_formacao='${e}'`).join(" OR ")})`);
        }

        if (filterObject?.saida_profissional) {
            const saida_profissional = filterObject.saida_profissional.split(",");
            filter.push(`(${saida_profissional.map(e => `saidas_profissionais.label='${e}'`).join(" OR ")})`);
        }

        if (filterObject?.inicio_candidatura && filterObject?.fim_candidatura) {
            const inicio = Date.parse(filterObject.inicio_candidatura);
            const fim = Date.parse(filterObject.fim_candidatura);

            filter.push(`(data_inicio_timestamp >= ${inicio / 1000} AND data_fim_timestamp <= ${fim / 1000})`);
        }

        //combina td filtro com and pa garanti que seja aplicado em conjunto
        const finalFilter = filter.join(" AND ");
        ...
        console.log("========================");
        console.log({searchResults: searchResults?.hits});
        console.log("========================");
